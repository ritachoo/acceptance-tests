import logging

from content import content_vars
from testing_projects_common.blocks.base_block import BaseBlock
from testing_projects_common.blocks.link import Link


class LoginBlock(BaseBlock):
    block_attribute_name = content_vars.test_attr_name
    block_attribute_value = "login-link"

    def get_login_link(self) -> Link:
        return Link(self.me)

    def click_login_link(self):
        logging.info("Click on login link in header block")
        link = self.get_login_link()
        link.wait_for_ready()
        link.click()

...
from acceptance_core.helpers.utils.collections_utils import (
    check_collection_is_not_empty,
)
from acceptance_core.helpers.utils.collections_utils import is_exist_index_in_list
...
class Row(BaseBlock):
    ...
    def get_cell_by_index(self, index: int) -> Cell | None:
        ...
        if not is_exist_index_in_list(cells, index):
            raise UTException(f"Not exist {index=} in {cells=}")
        return cells[index]
